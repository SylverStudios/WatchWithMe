...
var VideoHistory = function () {
  function VideoHistory() {
    _classCallCheck(this, VideoHistory);

    this._queue = [];
  }

  _createClass(VideoHistory, [{
    key: 'add',
    ...
    value: function add(state) {
      var videoTime = (0, _numeral2.default)(state.time).format('00:00');
      var historyEntry = state.last_action.initiator;
...
var _phoenix = __webpack_require__(9);
...
var connect = function connect(address, room) {
  var username = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : getRandomInt();


  var socket = new _phoenix.Socket(address, { params: { username: username } });
  socket.connect();
...
var _channelConnect = __webpack_require__(4);

var _VideoHistory = __webpack_require__(1);

var _VideoHistory2 = _interopRequireDefault(_VideoHistory);
...
var address = 'ws://' + serverIp + ':4000/socket';
var channel = void 0;
var tabId = void 0;
var videoHistory = new _VideoHistory2.default();

// Content scripts are tab specific because they are linked to a DOM
var sendMessageToContentScript = function sendMessageToContentScript(message) {
  if (tabId) {
 ...
 * 3. Handle messages from channel
 */
var connect2Channel = function connect2Channel(address, room) {
  return chrome.identity.getProfileUserInfo(function (userInfo) {
    var myUsername = userInfo.email.split('@')[0];
    channel = (0, _channelConnect.connect)(address, room, myUsername);
    channel.on("user_joined", handleUserJoin);
    channel.on("state_change", handleStateChange);
  });
};

var handleUserJoin = function handleUserJoin(channelPayload) {
  videoHistory.addMessage(channelPayload.body);
};

var handleStateChange = function handleStateChange(channelPayload) {
 ...
 */
var handleMessageFromContentScript = function handleMessageFromContentScript(message) {
